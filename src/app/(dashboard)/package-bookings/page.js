...
function getPackageGuests(booking) {
  const adults = Number(
    booking?.total_adults ??
    booking?.adults ??
    booking?.pax?.adults ??
    booking?.package_details?.adults ??
    0
  );
  const children = Number(
    ...
    booking?.package_details?.children ??
    0
  );
...
function getPackageLocation(booking) {
  return (
    booking?.package_details?.destination ||
    booking?.package_details?.city ||
    booking?.package_details?.location ||
    booking?.package_category ||
    "Package destination"
  );
}

function canCancelPackageBooking(booking) {
  const startDate =
    booking?.package_details?.startDate ||
    booking?.package_details?.start_date ||
    booking?.booking_criteria?.start_date ||
...
function getPackageDateValue(booking) {
  return (
    booking?.package_details?.startDate ||
    booking?.package_details?.start_date ||
    booking?.booking_criteria?.start_date ||
  ...
  async function fetchBookings() {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/b2c/dashboard/package/bookings`,
        {
          method: "POST",
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((booking) => {
      ...
      if (dateRange[0] || dateRange[1]) {
        const startDateRaw = getPackageDateValue(booking);
        if (!startDateRaw || !moment(startDateRaw).isValid()) return false;
        const startMoment = moment(startDateRaw);
        if (dateRange[0] && startMoment.isBefore(moment(dateRange[0]), "day"))
          return false;
        if (dateRange[1] && startMoment.isAfter(moment(dateRange[1]), "day"))
          return false;
      }
  ...
  return (
    <>
      <CancelBookingModal
        ...
        type="package"
        endPoint="/api/packages/cancel-booking"
      ...
      <div className="hb-page">
        <div className="dash-topbar">
          <div>
            <h4 className="dash-topbar-title">Package Bookings</h4>
          ...
          <div className="hb-cards">
            {loading ? (
              ...
              paginatedBookings.map((booking, index) => {
                ...
                const guests = getPackageGuests(booking);
                const dateValue = booking?.package_details?.startDate
                  ? moment(booking.package_details.startDate).format(
                    "MMM DD, YYYY"
                  )
                  : booking?.package_details?.start_date
                    ? moment(booking.package_details.start_date).format("MMM DD, YYYY")
                    : moment(booking?.created_at).format("MMM DD, YYYY");
                const voucherHref = `/packages/voucher/${booking?.booking_reference}`;
                const invoiceHref = `/packages/invoice/${booking?.invoice_number}`;
                const canCancel = canCancelPackageBooking(booking);
                const pax = (booking?.room_selections || []).reduce(
                  (total, room) =>
                    total +
                    (room.adultsInRoom || 0) +
                      ...
                      <div className="hb-card-top-left">
                        ...
                        <div>
                          <h6 className="hb-hotel-name">
                            {booking?.package_details?.title || "Package Booking"}
                          </h6>
                          {/* <p className="hb-hotel-location">
                            <FaMapMarkerAlt size={11} />
                            <span>{getPackageLocation(booking)}</span>
                          </p> */}
                        </div>
                      </div>
                    ...
                    <div className="hb-card-footer">
                      <div>
                        ...
                        <div className="hb-amount">
                          {booking?.customer_currency || ""} {booking?.amount ?? "-"}
                        </div>
                        <div className={`hb-status hb-status-${tone}`}>
                          Payment:{" "}
